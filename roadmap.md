...
> **定位**：面向个人和中小企业的网络安全工具，双版本并行：微信小程序（轻量拉新）+ PC 客户端（功能做深）。核心能力覆盖局域网服务发现、服务指纹识别、安全弱点检测、公网资产侦察，并以 DeepSeek 大模型驱动 AI 增值功能层。

> **架构演进**：
> - **2023-2024**: CLI/TUI 阶段，原型验证
> - **2025**: 微信小程序版立项，扫码即用的轻量方案
> - **2026**: PC 客户端版启动，Tauri + Rust 桌面化，双版本并行
...
**小程序版**（Phase 1）：轻量拉新，受微信 API 限制（白名单端口/mDNS），Rust 核心编译为 WASM。

**PC 版**（Phase 1+）：功能做深，无限制，Rust 核心直接集成，支持全端口扫描 + 工具聚合（nmap/masscan/nuclei/ffuf）。

Probe Agent 在 Phase 1 保留为小程序版全端口扫描的可选方案，不在 Phase 1 实现。

### 2.3 技术选型：Rust 统一语言

**选 Rust 的理由**：

- Tauri 原生语言，无 FFI 开销（PC 版直接调用 Rust Core）
- 小程序端 WASM 产物更小（Rust WASM 比 Go WASM 小 30-50%）
- tokio 异步并发成熟，网络扫描性能与 Go 持平
- 统一技术栈，减少维护负担

**Probe Agent 定位**：Phase 2+ 小程序版全端口扫描的可选方案，Phase 1 不实现。
